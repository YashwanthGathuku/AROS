    ...
    fn serve_once(status: &'static str, body: &'static str) -> String {
        ...
        thread::spawn(move || {
            if let Some(mut stream) = listener.incoming().flatten().next() {
                ...
                // Drain the request until the client half-closes. Closing with
                // unread bytes remaining makes Windows RST (WSAECONNRESET),
                // which hides a completed HTTP response from the caller.
