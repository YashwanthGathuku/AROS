"""Runtime loader for the JSON ResearchSkill catalog."""
...
class SkillCatalog:
    ...
    def _load(self) -> dict[str, ResearchSkill]:
        ...
        for path in sorted(self.root.glob("*.json")):
            skill = ResearchSkill.model_validate(json.loads(path.read_text(encoding="utf-8")))
    ...
    def applicable(self, visibility: str, facts: set[str]) -> tuple[ResearchSkill, ...]:
        ...
        return tuple(
            ...
            if (not skill.applicability or visibility in {value.lower() for value in skill.applicability})
    ...
    def select(self, visibility: str, facts: set[str]) -> ResearchSkill:
        ...
        return sorted(candidates, key=lambda skill: (-len(skill.required_facts), skill.id))[0]
